    ...
    for course in courses:
        ...
        for i in range(questions_per):
            ...
            if results.iloc[i]['Zero-Shot Evaluation'] == 1:
                print('no few shot needed for ' +
                      course + ' question ' + str(i+1))
                few_shot_input = 'n/a'
                few_shot_output = 'n/a'
            ...
            # incorrect via zero-shot:
                ...
                few_shot_input = ''
                print('doing few-shot for ' + course +
                      ' question ' + str(i+1) + '...')
...
def execute_GPT3_CoT_one_shot(courses, questions_per):
    ...
    for course in courses:
        ...
        for i in range(questions_per):
            closest_index = int(
                results.iloc[i]["Most Similar Questions"].strip('][').split(', ')[0]) - 1
            ...
            print("Running GPT-3 CoT one-shot on " +
                  course + ' question ' + str(i+1) + '...')
            start = time.time()
            # to avoid an openai.error.RateLimitError
...
if __name__ == "__main__":
    if args.Do_Courses:
        ...
        if args.GPT3_CoT_One_Shot:
            execute_GPT3_CoT_one_shot(
                courses_to_few_shot, questions_per_course)
    if args.Do_MATH:
        if args.Codex_Few_Shot:
            execute_few_shot(MATH_sections_to_few_shot,
                             questions_per_MATH_section)
        if args.GPT3_CoT_One_Shot:
            execute_GPT3_CoT_one_shot(
                MATH_sections_to_few_shot, questions_per_MATH_section)
